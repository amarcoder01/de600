...
const ResultsTable: React.FC<ResultsTableProps> = ({
  ...
  const formatNumber = (value: number | undefined, decimals = 2): string => {
    ...
    if (value === 0 && decimals > 0) return '0.00'; // Show 0.00 for zero values with decimals
  ...
  const formatMarketCap = (value: number | undefined): string => {
    if (value === undefined || value === null || isNaN(value)) return 'N/A';
    if (value === 0) return '$0'; // Show $0 for zero market cap
  ...
  const columns = [
    ...
    { field: 'market_cap' as keyof ScreenerStock, label: 'Market Cap', width: 'w-28', hideOnMobile: true },
    { field: 'exchange' as keyof ScreenerStock, label: 'Exchange', width: 'w-20', hideOnMobile: true },
  ];
        ...
        <div className="min-w-full">
          <table className="w-full text-xs sm:text-sm">
            <thead className="bg-muted/50 sticky top-0 z-10">
              <tr>
                {columns.map((column) => (
                  <th
                    key={column.field}
                    className={`${column.width} px-2 sm:px-3 lg:px-6 py-2 sm:py-3 text-left text-[11px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider cursor-pointer hover:bg-muted transition-colors ${column.hideOnMobile ? 'hidden sm:table-cell' : ''}`}
            ...
            <tbody className="bg-card divide-y divide-border">
              {stocks.length === 0 ? (
                <tr>
                  <td colSpan={columns.length} className="px-2 sm:px-3 lg:px-6 py-12 text-center text-muted-foreground">
                ...
                stocks.map((stock) => (
                  <tr key={stock.ticker} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-2 sm:px-3 lg:px-6 py-2 sm:py-3 lg:py-4">
                      <div className="text-sm text-foreground truncate max-w-28 sm:max-w-32 lg:max-w-48" title={stock.name}>
                        {stock.name}
                      </div>
                    </td>
                    <td className="px-2 sm:px-3 lg:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap">
