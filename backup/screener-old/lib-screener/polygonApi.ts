...
class PolygonApiService {
  ...
  private readonly MIN_REQUEST_DELAY = 1200; // 1.2 seconds between requests (50 requests/minute max)
  ...
  private async waitForRateLimit(): Promise<void> {
    ...
    if (timeSinceLastRequest > this.RATE_LIMIT_RESET_TIME) {
      this.requestCount = 0;
    }
    
    // If we've made too many requests, wait
    if (this.requestCount >= this.MAX_REQUESTS_PER_MINUTE) {
      const waitTime = this.RATE_LIMIT_RESET_TIME - timeSinceLastRequest;
      if (waitTime > 0) {
  ...
  private hasValidApiKey(): boolean {
    const isValid = !!(this.apiKey && this.apiKey.trim() !== '' && this.apiKey !== 'your_polygon_api_key_here');
    if (!isValid) {
      console.warn('🔑 Polygon API key not configured properly. Using mock data fallback.');
    }
    return isValid;
  }

  // Generate mock stock data for fallback when API is unavailable
  private generateMockStockData(ticker: string, name?: string): ScreenerStock {
    // Generate realistic mock data based on ticker
    const mockPrice = Math.random() * 200 + 10; // $10-$210
    const mockChange = (Math.random() - 0.5) * 10; // -$5 to +$5
    const mockChangePercent = (mockChange / mockPrice) * 100;
    const mockVolume = Math.floor(Math.random() * 1000000) + 100000; // 100K-1.1M
    const mockMarketCap = mockPrice * (Math.random() * 100000000 + 10000000); // 10M-100M shares
    ...
    return {
      ...
      change_percent: Math.round(mockChangePercent * 100) / 100,
      volume: mockVolume,
      market_cap: Math.round(mockMarketCap),
  ...
  async getTickers(filters?: FilterCriteria, limit: number = 100): Promise<Stock[]> {
    if (!this.hasValidApiKey()) {
      console.warn('No valid API key available, generating mock ticker data');
      // Return mock ticker data when API key is not available
      const mockTickers = [
        'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'AMD', 'INTC',
        'ORCL', 'CRM', 'ADBE', 'PYPL', 'UBER', 'SPOT', 'ZOOM', 'SQ', 'TWTR', 'SNAP',
        'BABA', 'TSM', 'ASML', 'AVGO', 'TXN', 'QCOM', 'COST', 'TMUS', 'CMCSA', 'PEP'
      ].slice(0, limit);
      ...
      return mockTickers.map(ticker => ({
        ...
        sic_description: 'Technology Services'
      }));
    }
    ...
    try {
      ...
      const response = await this.makePolygonRequest('/v3/reference/tickers', params);
      ...
      // Handle rate limit exceeded error by returning mock data
      if (error instanceof Error && error.message === 'API_RATE_LIMIT_EXCEEDED') {
        console.warn('🚦 API rate limit exceeded, generating mock ticker data');
        const mockTickers = [
          'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'AMD', 'INTC',
          'ORCL', 'CRM', 'ADBE', 'PYPL', 'UBER', 'SPOT', 'ZOOM', 'SQ', 'BABA', 'TSM'
        ].slice(0, limit);
  ...
  async getStockPrice(ticker: string, includeFinancials: boolean = true): Promise<StockPrice> {
    if (!this.hasValidApiKey()) {
      console.warn(`No valid API key, generating mock price data for ${ticker}`);
      const mockData = this.generateMockStockData(ticker);
      return {
    ...
    try {
      ...
      for (const ticker of tickers) {
        ...
        const stock: ScreenerStock = {
          ...
          sector: ticker.sic_description ? mapSicToSector(ticker.sic_description) : undefined,
  ...
  async searchMarketSnapshot(
    ...
    // Return full hydrated list; API route will sort and paginate using offset/limit
    return { stocks: hydrated, totalCount, hasMore: false, date: dateISO };
  }
  ...
  private validateStockData(stock: any): stock is ScreenerStock {
    ...
    // Price validation - be more lenient and provide fallbacks
    if (stock.price !== undefined && stock.price !== null) {
      if (typeof stock.price !== 'number' || isNaN(stock.price) || stock.price < 0) {
        console.warn(`Invalid price for ${stock.ticker}: ${stock.price}, setting to 0`);
        stock.price = 0; // Set to 0 instead of undefined
      }
    } else {
      stock.price = 0; // Default to 0 if no price available
    }
    
    // Volume validation - provide fallback
    if (stock.volume !== undefined && stock.volume !== null) {
      if (typeof stock.volume !== 'number' || isNaN(stock.volume) || stock.volume < 0) {
        console.warn(`Invalid volume for ${stock.ticker}: ${stock.volume}, setting to 0`);
        stock.volume = 0;
      }
    } else {
      stock.volume = 0; // Default to 0 if no volume available
    }
    
    // Change validation - keep as is if invalid (undefined is acceptable for display)
    ...
    // Change percent validation - keep as is if invalid
    ...
    // Market cap validation - keep as is if invalid (undefined is acceptable)
  ...
  // Enhanced error handling for API calls with rate limiting
  private async makePolygonRequest(endpoint: string, params: any = {}, timeout: number = 10000, retries: number = 3): Promise<any> {
      ...
      try {
        ...
        if (axios.isAxiosError(error)) {
          if (error.response?.status === 429) {
            console.warn(`🚦 Rate limit hit on attempt ${attempt}/${retries}. Waiting before retry...`);
            if (attempt < retries) {
              // Exponential backoff for rate limit errors
              const backoffDelay = Math.min(30000, 5000 * Math.pow(2, attempt - 1)); // Max 30s
              await new Promise(resolve => setTimeout(resolve, backoffDelay));
              continue; // Retry the request
            } else {
              console.error('🚫 Rate limit exceeded after all retries. Using fallback data.');
            ...
            if (attempt < retries) {
              console.warn(`⏰ Request timeout on attempt ${attempt}/${retries}. Retrying...`);
        ...
        if (attempt < retries) {
          console.warn(`⚠️ Request failed on attempt ${attempt}/${retries}. Retrying...`);
...
export const fetchAllUSStocks = async (
  ...
  try {
    ...
    if (!API_KEY || API_KEY === 'your_polygon_api_key_here') {
      console.log('📊 No valid API key found, generating mock stock data');
      const mockTickers = [
        'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'AMD', 'INTC',
        'ORCL', 'CRM', 'ADBE', 'PYPL', 'UBER', 'SPOT', 'ZOOM', 'SQ', 'TWTR', 'SNAP',
        'BABA', 'TSM', 'ASML', 'AVGO', 'TXN', 'QCOM', 'COST', 'TMUS', 'CMCSA', 'PEP',
        'JNJ', 'WMT', 'JPM', 'V', 'PG', 'UNH', 'MA', 'DIS', 'HD', 'NFLX'
      ].slice(0, limit);

      const mockStocks: ScreenerStock[] = mockTickers.map(ticker => {
        const mockPrice = Math.random() * 200 + 10;
        const mockChange = (Math.random() - 0.5) * 10;
        const mockChangePercent = (mockChange / mockPrice) * 100;
        const mockVolume = Math.floor(Math.random() * 1000000) + 100000;
        const mockMarketCap = mockPrice * (Math.random() * 100000000 + 10000000);
      ...
      if (onProgress) {
        onProgress(mockStocks.length, mockStocks.length, 'Generated mock stock data');
      }
    ...
    if (!result.stocks || result.stocks.length === 0) {
      console.log('⚠️ No stocks returned from API, generating fallback data');
      // Generate fallback data if API returns no results
      const fallbackTickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA'].slice(0, limit);
      const fallbackStocks: ScreenerStock[] = fallbackTickers.map(ticker => {
        ...
        return {
          ...
          market_cap: Math.round(mockPrice * 100000000),
          sector: 'Technology',
          exchange: 'NASDAQ'
        };
      });
      return { stocks: fallbackStocks, hasMore: false };
    }
