...
export class ScreenerDataService {
  ...
  async getUnifiedSnapshot(ticker: string, includeFinancials = true): Promise<UnifiedSnapshot> {
    ...
    const snapshot: UnifiedSnapshot = {
      ticker,
      price: has_price && typeof price === 'number' && price > 0 ? price : 0, // Ensure valid price or default to 0
      change: typeof change === 'number' ? change : undefined,
      change_percent: typeof change_percent === 'number' ? change_percent : undefined,
      volume: typeof volume === 'number' && volume >= 0 ? volume : 0, // Default volume to 0 if invalid
      market_cap: typeof market_cap === 'number' && market_cap > 0 ? market_cap : undefined,
      has_change,
      market_state,
      basis,
    };
